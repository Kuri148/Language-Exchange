...
namespace Yodokorochan
{
    ...
    public class Yodo_TagMarker : UdonSharpBehaviour
    {
        ...
        [Header("頭上オフセット")]
        public float Yodo_MarkerOffset = 0.35f;
        ...
        private Quaternion my_canvas_rotation = Quaternion.Euler(0, 180, 0);
        ...
        private void Start()
        {
            ...
            int cur = 0;
            foreach (Transform child in Yodo_MarkersRoot.transform)
            {
                if (MAX_PLAYERS_IN_INSTANCE <= cur) { break; }
                markers[cur] = child.gameObject;
                markers[cur].SetActive(false);
                cur++;
            }
            for (cur = 0; cur < MAX_PLAYERS_IN_INSTANCE; cur++)
            {
                    ...
                    if (playerApis[cur].IsValid())//自分がワールド退出する時のエラー対策
                    {
                        if (Yodo_TagMarker_Active && (Yodo_TagStatus[cur] != 0UL))
                        {
                            ...
                            if (head_pos != null)
                            {
                                ...
                                {
                                    markers[cur].transform.position = head_pos + head_offset;
                                }
                                else
                                {
                                    markers[cur].transform.position = playerApis[cur].GetPosition() + generic_offset;
                                }
                            }
                            else
                            {
        ...
        private void _UpdateTags()
        {
            ...
            for (int cur = 0; cur < MAX_PLAYERS_IN_INSTANCE; cur++)
            {
                if (recentPlayerIds[cur] != Yodo_PlayerIds[cur])
                {
                    ...
                    {
                        playerApis[cur] = null;
                        markers[cur].transform.localPosition = Vector3.zero; // 被ってるけど念のため
                        markers[cur].SetActive(false);
                    }
                    else
                    {
